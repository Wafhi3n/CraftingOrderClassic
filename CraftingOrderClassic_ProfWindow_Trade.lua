...
--
-- La silhouette est celle du panneau d'échange (_Enchant_Trade_Ask:BuildSilhouette) : même
-- disposition, même clic — demander la pièce au partenaire (chuchotement + ASKE).
--
...
local function wanted(self)
    if not (self.docked and self.frame and self.frame:IsShown()) then return false end
    if not (_G.TradeFrame and TradeFrame:IsShown()) then return false end
    local C = COC.Craft
    if not (C and C:OpenProfessionKey() == "Enchanting") then return false end
...
function PW:_SyncTradeView()
    ...
    if wanted(self) then
        ...
        if self.dockView ~= "trade" then self:_SetDockView("trade") else self:_FillTradeView() end
        return
    end
...
function PW:_BuildTradeView()
    ...
    Skin.SkinWell(well)
    local sil = Ask:BuildSilhouette(well)
