...
local ROW_H = 18
-- Exposee : le bloc FOURNITURES vit dans _Route_Supply.lua et mesure ses hauteurs sur la meme
-- trame. Une constante recopiee dans deux fichiers finit toujours par diverger.
PW.ROUTE_ROW_H = ROW_H
...
-- Le TTL est une minuterie AVEUGLE : changer d'onglet relit le CACHE, pas la fenêtre — un aller-retour
-- rapide rejoue la même réponse périmée (relevé 2026-09-23 : route « Rough Copper Bomb » alors que
-- Coarse Dynamite venait d'être apprise). D'où la péremption sur ÉVÉNEMENT, le chrono ne restant qu'un
-- plafond. ⚠️ MARQUAGE PUR, aucun appel de frame : c'est ce qui le rend appelable depuis un
-- gestionnaire d'événement (greffée, la colonne est enfant de `ProfessionsFrame` — y repeindre serait
-- Show/Hide sur un enfant protégé). Le repaint déjà en place (`PW:Refresh`, débouncé 0,1 s) relira.
function PW:InvalidateDockRecipes()
    self._dockRecipesAt = nil
end
...
function PW:_FillRoute()
    ...
    if not route then
        ...
        f.msg:SetText(COC.Profit and COC.Profit:NoPriceMessage() or ""); f.msg:Show()
        return
    end
